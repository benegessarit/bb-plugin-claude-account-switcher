...
- Let `claude auth login --claudeai` own authorization, but route its browser URL through one short-lived `BROWSER` launcher that invokes Chrome with `--incognito --new-window`. Claim the first automatic invocation atomically so later automatic callbacks are successful no-ops. While that same login is pending, allow an explicit **Return to authorization** action to reopen only the exact captured consent URL. Chrome owns process and OS-window reuse. Never use `open -n`, create a temporary browser profile, or collect an email address.
